...
/*   Updated: 2022/11/06 22:46:15 by hacho            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_putchar_repeat(' ', context->min_field_width - length - \
		(context->precision > length) * (context->precision - length) - \
		(sign != 0), STDOUT_FILENO) == -1)
		return (-1);
	if (sign != 0)
		ft_putchar_fd(sign, STDOUT_FILENO);
	if (ft_putchar_repeat('0', context->precision - length, \
		STDOUT_FILENO) == -1)
		return (-1);
	if (length != 0)
